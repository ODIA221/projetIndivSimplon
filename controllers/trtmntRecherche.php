...
/* $inputs = $bd -> prepare('SELECT * FROM utilisateur  WHERE  etat = 1');
$inputs -> execute();
...
if (isset($_GET['recherche'])) {
        ...
        if (!empty($_GET['recherche'])) {
                ...
                //Requête de recherche
                $rows = $bd -> query("SELECT * FROM utilisateur where etat = 1 and (matricule like '%$recherche%' or nom like '%$recherche%' or prenom like  '%$recherche%'  or mail like '%$recherche%' or roles like '%$recherche%')" );
                header("location: ../views/afficherRecheche.php");
                exit;
